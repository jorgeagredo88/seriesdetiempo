...
## Serie de tiempo por Fecha de Origen

Se agrupa el monto en dólares por la variable objetivo.

```{r}
Datos_FO <- aggregate(Monto_USD ~ Fecha_Origen, data = Datos, sum)
...
```{r echo=FALSE}
plot(serie_xts1, col = "blue", main = "Serie de Tiempo: Monto USD", ylab = "Monto USD", xlab= "Fecha de Origen")
```
...
```{r echo=FALSE}
fecha_corte <- as.Date("2023-10-01")

Datos_FOF <- Datos_FO[Datos_FO$Fecha_Origen >= fecha_corte, ]

# Crear la serie de tiempo
serie_xts2 <- xts(Datos_FOF$Monto_USD, order.by = Datos_FOF$Fecha_Origen)

```

```{r echo=FALSE}
plot(serie_xts2, col = "blue", main = "Serie de Tiempo: Monto USD", ylab = "Monto USD", ylab= "Fecha de Origen")
```
...
```{r echo=FALSE}
# Gráfico de rezago para la serie 
lag.plot(serie_xts2, lags = 12, layout = c(4, 3), main = "Gráfico de Rezago para la Serie de Tiempo")
...
m_movil_10 <- rollmean(Datos_FOF$Monto_USD, k = 10, fill = NA, align = "right")
m_movil_30 <- rollmean(Datos_FOF$Monto_USD, k = 30, fill = NA, align = "right")

plot(Datos_FOF$Fecha_Origen, Datos_FOF$Monto_USD, 
     type = "l", 
     col = "black", 
     lwd = 1, 
     xlab = "Fecha de Origen", 
     ylab = "Monto USD", 
     main = "Monto USD con Medias Móviles Simples de 10 y 30 días")

# Añadir la media móvil simple de 10 días
lines(Datos_FOF$Fecha_Origen, m_movil_10, col = "blue", lwd = 2)

# Añadir la media móvil simple de 30 días
lines(Datos_FOF$Fecha_Origen, m_movil_30, col = "red", lwd = 2)
...
# Calcular la media móvil exponencial de 10 días
ema1_10 <- EMA(Datos_FOF$Monto_USD, n = 10)

# Calcular la media móvil exponencial de 30 días
ema1_30 <- EMA(Datos_FOF$Monto_USD, n = 30)
...
# Añadir la media móvil exponencial de 10 días
lines(Datos_FOF$Fecha_Origen, ema1_10, col = "blue", lwd = 2)

# Añadir la media móvil exponencial de 30 días
lines(Datos_FOF$Fecha_Origen, ema1_30, col = "red", lwd = 2)
